...
### Durable hosting migration (2026-07-28) — IN PROGRESS, READ FIRST

User requirement: deployment must NOT depend on a particular Kimi agent instance (kimi.page URLs are agent-tied). New architecture (user approved):

- **App**: `https://kire321.github.io/` served from repo `kire321.github.io` (root hosting; SW scope `/`, offline images fully work). Deploy workflow: `deploy-app-rootrepo.yml` staged in sandbox `/mnt/agents/output/deploy/` — it checks out public `kire321/thai-rpg@master`, builds, deploys. Triggers: dispatch (PAT has actions:write) + daily schedule.
- **CMS**: `https://kire321.github.io/thai-rpg-content/` — repo `kire321/thai-rpg-content`, workflow `.github/workflows/deploy-cms.yml` (already on its `main`) publishes `public/` (full 16MB episodes.json + all images). Same origin as the app => no CORS issues.
- **App code** (master, commit 58f16de1): `DEFAULT_CMS_BASE='/thai-rpg-content'` (same-origin relative), all asset/manifest/sw paths relative (works at root AND subpath), bundled fallbacks (episodes/vocab/icon) now COMMITTED so builds are self-contained, `CACHE_NAME` `thai-rpg-2026-07-28-02`.
- **Cloudflare proxy** (production URL): once `api.workers.cloudflare.com` recovers, plain passthrough `_worker.js` to `https://kire321.github.io` — root hosting means `/thai-rpg-content/*` needs no special-casing. Cron retry (every 2h) does this automatically; full runbook in sandbox `/mnt/agents/output/deploy/STATE.md`.

**Pending user actions** (then finish per STATE.md): (1) PAT needs Pages **write** (POST /pages 403 — grant Pages: Read and write and SAVE, or enable Pages in each repo's dashboard: Settings -> Pages -> Source: GitHub Actions); (2) create empty public repo `kire321.github.io` (exact name) and add it to the PAT's repo list.

Fallback target if GH Pages isn't ready when Cloudflare recovers: kimi.page build `z4vwnuwfhj43i` (version 9e43e10, complete + verified, CACHE_NAME thai-rpg-2026-07-28-01).
