...
function sm2Schedule(quality, repetitions, interval, ef) {
  ...
  if (quality < 3) {
    ...
    if (repetitions === 0) {
      newInterval = Math.max(1, Math.ceil(2 * qualityScale));
    } else if (repetitions === 1) {
      newInterval = Math.max(1, Math.ceil(5 * qualityScale));
    } else {
...
// NEW: Support acts with multiple tags (act.tags: string[]) or single tag (act.tag: string).
...
// Extract numeric portion from episode ID for tiebreaking (e.g. "ep_042" → 42)
function getEpisodeNumber(episodeId) {
...
function getNextEpisode(state, env) {
  ...
  // EPISODE SELECTION ALGORITHM v3:
  ...
  // 6. Tiebreak: highest episode ID (newest first) — speeds up testing new content

  if (cards.length === 0) {
    return state.episodes.sort((a, b) => getEpisodeNumber(b.id) - getEpisodeNumber(a.id))[0];
  }
  ...
  // (Now supports multiple tags per act — checks all tags in the first act)
  let candidateEpisodes = [];
  for (const episode of state.episodes) {
    const firstAct = episode.acts?.[0];
    const firstActTags = getActTags(firstAct);
    if (firstActTags.length > 0 && firstActTags.some(tag => matchingTags.includes(tag))) {
  ...
  if (candidateEpisodes.length === 0) {
    ...
    fallback.sort((a, b) => getEpisodeNumber(b.id) - getEpisodeNumber(a.id));
    return fallback[0];
  }

  // Step 5: Prioritize episodes reviewing the most due cards
  // Step 6: Tiebreak: highest episode ID (newest first)
  candidateEpisodes.sort((a, b) => {
    ...
    return getEpisodeNumber(b.id) - getEpisodeNumber(a.id); // Newest first
  });
...
function calculateCounters(state, env) {
  ...
  // This counter is incremented in onRateCard when a card has
  // lastReviewed === null (truly new). It correctly handles:
  // - New cards rated "again": counted as new (they were new when first shown)
  // - Due cards rated "good": NOT counted as new (they were due, not new)
  const newToday = state.newCardsRatedToday || 0;
...
const Handlers = {
  ...
  onTapNextLine: (state, env) => {
    ...
    if (phase === 'lines_before') {
      ...
      if (isOverpressure) {
        ...
        if (!hasDueCardsForTag) {
          // Skip vocab review — no due cards for this act's tags while overpressure
          return {
    ...
    if (phase === 'lines_after') {
      ...
      const episode = getCurrentEpisode(state);
      if (episode && nextActIndex < getTotalVirtualActs(episode)) {
        return {
  ...
  onTapChoice: (state, env, choiceIndex) => {
    ...
    // Pseudo-random using sin-based hash of timestamp + counter.
    // This works even in environments where Math.random() is deterministic.
 ...
 // Mark as played today when episode completes (last act)
  ...
  onRateCard: (state, env, cardId, quality) => {
    ...
    const newCardsRatedToday = (state.newCardsRatedToday || 0) + (isNewCard ? 1 : 0);
    ...
    return { cardStats: newStats, showingAnswer: false, currentCardIndex, againQueue: newAgainQueue, againDelayCounter, newCardsRatedToday };
  },
