...
const DEFAULT_CMS_BASE = 'https://q4kgqw3jj72wa.kimi.page';
...
// Normalize episode lines so stage_directions is always string[].
...
// double-normalization bugs with saved state.
export function normalizeEpisode(ep: any): any {
  if (!ep) return ep;
  const normalized = { ...ep };
  if (Array.isArray(normalized.acts)) {
    normalized.acts = normalized.acts.map(normalizeActLines);
  }
  return normalized;
}

// Normalize lines in an act (stage_directions string → array).
// Does NOT expand segments — that's handled on-the-fly by the controller.
function normalizeActLines(act: any): any {
  if (!act) return act;
  const normalized = { ...act };
  if (Array.isArray(normalized.lines_before)) {
    normalized.lines_before = normalized.lines_before.map(normalizeLine);
  }
  if (Array.isArray(normalized.lines_after)) {
    normalized.lines_after = normalized.lines_after.map(normalizeLine);
  }
  if (act.decision) {
...
export class Store extends Component<{}, StoreState> {
  ...
  async loadAllContent() {
    ...
    // Prefetch all images so the app works fully offline
    this.prefetchImages(charactersMap, placesMap);
  }
  ...
  // Prefetch all character/place images so the app works fully offline
  private prefetchImages(charactersMap: Record<string, any>, placesMap: Record<string, any>) {
    const cmsBaseUrl = this.state.cmsBaseUrl || 'https://ipozfyeyt26ay.kimi.page';
    const imageUrls: string[] = [];

    // Collect all picture URLs from characters
    for (const char of Object.values(charactersMap)) {
      if (char && char.picture) {
        const url = char.picture.startsWith('/') ? `${cmsBaseUrl}${char.picture}` : char.picture;
        imageUrls.push(url);
      }
    }
    // Collect all picture URLs from places
    for (const place of Object.values(placesMap)) {
      if (place && place.picture) {
        const url = place.picture.startsWith('/') ? `${cmsBaseUrl}${place.picture}` : place.picture;
        imageUrls.push(url);
      }
    }

    if (imageUrls.length === 0) return;
    console.log(`[Images] Prefetching ${imageUrls.length} images for offline use...`);

    // Fetch all images in parallel with CORS mode.
    // Using mode: 'cors' (not 'no-cors') ensures the SW caches non-opaque
    // responses that the browser can decode when served offline.
    // The <img> tags also use crossOrigin="anonymous" which sends CORS
    // requests — both must match for the SW cache to hit.
    Promise.all(
      imageUrls.map(url =>
        fetch(url, { mode: 'cors' })
          .then(() => true)
          .catch(() => {
            // CORS not supported by server — fall back to no-cors
            // (images will work online but not offline)
      ...
      console.log(`[Images] Prefetched ${successCount}/${imageUrls.length} images`);
    });
  }
