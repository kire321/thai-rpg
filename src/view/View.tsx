...
const imageDiagnostics: { lastHidden?: number; lastShown?: number; brokenImages: string[] } = {
  brokenImages: [],
};
...
if (typeof document !== 'undefined') {
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      ...
      // Check all images when returning to foreground
      setTimeout(() => {
        const images = document.querySelectorAll('img');
        imageDiagnostics.brokenImages = [];
...
const CachedImage: React.FC<{ src: string; alt: string }> = ({ src, alt }) => {
  ...
  // If load failed, show placeholder; retry on next render
  if (loadFailed) {
    return (
...
const InfoCard: React.FC<{
  ...
  return (
    <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col">
      {/* Label header */}
      <div className="px-3 py-2 bg-slate-200/50 dark:bg-slate-200 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
        <p className="text-xs uppercase tracking-wider text-slate-600 dark:text-slate-400">{label}</p>
      </div>
      {/* Picture — full width, taller */}
...
const LineCard: React.FC<{
  ...
  // Handle both object lines (with .dialogue) and string lines (outcome text)
  let dialogue = typeof line === 'string' ? line : (line.dialogue || '');
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-md mx-auto">
      ...
      {(() => {
        ...
        return (
          <div className="w-full mb-3 p-3 bg-slate-100/60 dark:bg-white dark:bg-slate-800/60 rounded-lg border-l-2 border-amber-500">
            {arr.map((s: string, i: number) => (
              <p key={i} className="text-amber-700/80 dark:text-amber-600 dark:text-amber-300/80 text-sm italic">{s}</p>
            ))}
          </div>
        );
      })()}
...
const QuizCard: React.FC<{
  ...
  // - thai-eng card: Thai is on the front → play when card appears
  // - eng-thai card: Thai is on the back → play when answer is revealed
  React.useEffect(() => {
    if (!currentCard) return;
    if (isThaiFront && currentCard.front) {
      onSpeakThai(currentCard.front);
    }
  }, [currentCard.id]);

  React.useEffect(() => {
    if (!currentCard || !showingAnswer) return;
    if (!isThaiFront && currentCard.back) {
      onSpeakThai(currentCard.back);
    }
  }, [showingAnswer]);
    ...
    <div className="flex flex-col items-center w-full max-w-md mx-auto">
      <div className="w-full bg-white dark:bg-slate-800 rounded-2xl shadow-xl overflow-hidden mb-4">
        <div className="flex items-center justify-between px-4 py-2 bg-slate-200 dark:bg-slate-700 border-b border-slate-300 dark:border-slate-600">
          {/* Speaker button: replay Thai pronunciation */}
          <button
            onClick={onSpeakCard}
            className="p-1.5 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors"
            title="Replay pronunciation"
          >
            <Volume2 className="w-4 h-4 text-slate-600 dark:text-slate-400" />
          </button>
          {/* Diagnostic: act tag + due date */}
          <div className="flex items-center gap-2">
            ...
            {quizCardDueDate && (
              <span className={`text-[10px] px-1.5 py-0.5 rounded ${
                quizCardDueDate === 'New card'
                  ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300'
                  : quizCardDueDate.includes('again')
      ...
      {!showingAnswer ? (
        <button onClick={onShowAnswer}
          className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-slate-900 dark:text-white font-semibold rounded-xl transition-colors flex items-center justify-center gap-2">
        ...
        <div className="w-full grid grid-cols-4 gap-2">
          {(['again', 'hard', 'good', 'easy'] as const).map((rating) => (
            <button key={rating}
              ...
              className={`py-2 px-1 rounded-xl text-slate-900 dark:text-white font-semibold text-xs leading-tight transition-colors ${
              ...
              <div className="text-slate-900/70 dark:text-slate-900 dark:text-white/70 text-[10px]">
                {schedulePreview ? (
                  rating === 'again' && schedulePreview.againIsDelay
                    ? `${schedulePreview.again} cards`
                    : `${schedulePreview[rating]}d`
                ) : '-'}
              </div>
            </button>
          ))}
...
const Footer: React.FC<{
  ...
  // Show ALL subplots, not just non-zero ones
    ...
    <footer className="fixed bottom-12 left-0 right-0 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 z-40">
      <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1 px-4 py-2 text-xs">
        {allSubplotIds.map((id) => {
          ...
          // Compact name: remove "subplot_" prefix and "The " prefix
          const fullName = (subplots[id]?.name || id);
          ...
          return (
            <span key={id} className="text-slate-600 dark:text-slate-400">
              {compactName}:
              <span className={`ml-0.5 font-bold ${score > 0 ? 'text-green-600 dark:text-green-400' : score < 0 ? 'text-red-600 dark:text-red-400' : 'text-slate-400 dark:text-slate-600'}`}>
                {score > 0 ? '+' : ''}{score}
              </span>
            </span>
          );
    ...
    <div className="fixed inset-0 z-50 bg-black/50" onClick={handleBackdropClick}>
      <div className="absolute bottom-0 left-0 right-0 bg-white dark:bg-slate-800 rounded-t-2xl shadow-2xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-4 pb-4 border-b border-slate-200 dark:border-slate-700">
          ...
          <button onClick={onClose} className="p-2 rounded-full hover:bg-slate-200 dark:bg-slate-700 transition-colors"><X className="w-5 h-5 text-slate-600 dark:text-slate-400" /></button>
        </div>
        ...
        <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700">
          ...
          <div className="flex items-center justify-center gap-4">
            <button onClick={onDecrementDateshift} className="p-2 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:bg-slate-600 rounded-lg"><Minus className="w-5 h-5 text-slate-900 dark:text-white" /></button>
            <span className="text-2xl font-bold text-slate-900 dark:text-white w-12 text-center">{dateshift}</span>
            <button onClick={onIncrementDateshift} className="p-2 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:bg-slate-600 rounded-lg"><Plus className="w-5 h-5 text-slate-900 dark:text-white" /></button>
          </div>
        </div>
        ...
        <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700">
          ...
          <input
            type="url"
            value={cmsBaseUrl}
            onChange={(e) => onChangeCmsBase(e.target.value)}
            placeholder="https://..."
          ...
          />
          <p className="text-xs text-slate-500 dark:text-slate-500 mt-1">Content will reload on next app start</p>
        </div>

        <div className="p-4 space-y-3">
          <button onClick={onCheckForNewContent} className="w-full flex items-center gap-3 px-4 py-3 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:bg-slate-600 rounded-lg text-left">
          ...
          <button onClick={onCheckForUpdates} className="w-full flex items-center gap-3 px-4 py-3 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:bg-slate-600 rounded-lg text-left">
          ...
          <button onClick={onExportState} className="w-full flex items-center gap-3 px-4 py-3 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:bg-slate-600 rounded-lg text-left">
          ...
          <button onClick={() => fileInputRef.current?.click()} className="w-full flex items-center gap-3 px-4 py-3 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:bg-slate-600 rounded-lg text-left">
          ...
          <button onClick={onTapResetState} className="w-full flex items-center gap-3 px-4 py-3 bg-red-600 hover:bg-red-700 rounded-lg text-left">
            <Trash2 className="w-5 h-5 text-slate-900 dark:text-white" /><span className="text-slate-900 dark:text-white font-semibold">Reset State</span>
          </button>
        </div>
          ...
          {diagnosticsOpen && (
            <div className="px-4 py-3 bg-white dark:bg-slate-800 text-sm space-y-3">
                ...
                {mostOverdueCardInfo ? (
                  <div className="space-y-1">
                    <p className="text-slate-700 dark:text-slate-300">
                      ...
                      {mostOverdueCardInfo.isAgain && (
                        <span className="ml-1 text-[10px] px-1.5 py-0.5 bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 rounded">AGAIN</span>
                      )}
                      {mostOverdueCardInfo.isNew && (
                        <span className="ml-1 text-[10px] px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 rounded">NEW</span>
                      )}
                    </p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      Due: {mostOverdueCardInfo.isNew ? 'New card' : mostOverdueCardInfo.isAgain ? 'Due now (again)' : `Day ${mostOverdueCardInfo.dueDate}`}
                    </p>
                    {mostOverdueCardInfo.tags.length > 0 && (
                      <p className="text-xs text-slate-500 dark:text-slate-400">
                        Tags: {mostOverdueCardInfo.tags.join(', ')}
                      </p>
                    )}
                  </div>
                ) : (
              ...
              <div className="border-t border-slate-200 dark:border-slate-700 pt-2">
                <p className="text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1">Next episode (fast forward)</p>
                {nextEpisodeInfo ? (
                  <div className="space-y-1">
                    <p className="text-slate-700 dark:text-slate-300 font-medium">{nextEpisodeInfo.title}</p>
                    <div className="flex flex-wrap gap-1">
                      {nextEpisodeInfo.acts.map((act: any) => (
                        <span key={act.actIndex} className="text-[10px] px-1.5 py-0.5 bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 rounded">
                          Act {act.actIndex}: {act.tag}
                        </span>
                      ))}
                    </div>
                  </div>
      ...
      <div className="min-h-screen bg-slate-100 dark:bg-slate-950 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 max-w-md text-center">
          ...
          <button onClick={() => window.location.reload()}
            className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-slate-900 dark:text-white rounded-lg transition-colors">
            Reload
          </button>
        </div>
      </div>
...
const ViewInner: React.FC<ViewProps> = (props) => {
  ...
  const cmsBaseUrl = props.cmsBaseUrl ?? 'https://ipozfyeyt26ay.kimi.page';
  ...
  // Footer always visible when subplots exist
  ...
  return (
    <div className={`min-h-screen bg-slate-100 dark:bg-slate-950 ${hasFooter ? 'pb-20' : ''}`}>
      ...
      {toast && (
        <ToastNotification toast={toast} onClear={() => getHandler('onClearToast')()} />
      )}
  ...
  <path d="M20 20 L44 20 L40 12 L24 12 Z" />
  <path d="M16 28 L48 28 L44 20 L20 20 Z" />
  <path d="M12 36 L52 36 L48 28 L16 28 Z" />
  <path d="M8 44 L56 44 L52 36 L12 36 Z" />
  <rect x="20" y="44" width="8" height="12" />
  <rect x="36" y="44" width="8" height="12" />
  <path d="M4 56 L60 56" />
</svg>
              <button onClick={getHandler('onStartEpisode')}
                className="px-6 py-3 bg-amber-600 hover:bg-amber-700 text-slate-900 dark:text-white font-semibold rounded-xl transition-colors mb-3">
                Start Episode
              </button>
        ...
        {currentView === 'episode' && !currentAct && (
          <div className="max-w-md mx-auto mt-8 text-center">
            <div className="p-8 bg-white dark:bg-slate-900 rounded-2xl">
              ...
              <button onClick={getHandler('onStartEpisode')}
                className="mt-4 px-4 py-2 bg-amber-600 hover:bg-amber-700 text-slate-900 dark:text-white rounded-lg transition-colors">
                Retry
              </button>
            </div>
          </div>
            ...
            {(actPhase === 'lines_before' || actPhase === 'lines_after') && (
              <>
                ...
                <button onClick={getHandler('onTapNextLine')}
                  className="w-full py-3 mt-4 bg-amber-600 hover:bg-amber-700 text-slate-900 dark:text-white font-semibold rounded-xl transition-colors flex items-center justify-center gap-2">
            ...
            {showVocabReview && (
              <>
                <div className="text-center mb-3">
                  <p className="text-xs text-blue-600 dark:text-blue-400 uppercase tracking-wider">Vocab Review</p>
                </div>
                <QuizCard currentCard={currentCard} showingAnswer={showingAnswer}
                  schedulePreview={schedulePreview}
                  currentActTag={props.currentActTag || null}
                  quizCardDueDate={props.quizCardDueDate || null}
                  onShowAnswer={getHandler('onShowAnswer')}
                  onRateCard={(id, q) => getHandler('onRateCard')(id, q)}
                  onSpeakThai={onSpeakThai}
                  onSpeakCard={getHandler('onSpeakCard')} />
                {!showingAnswer && currentCard && (
                  <button onClick={getHandler('onVocabReviewDone')}
                    className="w-full mt-4 py-2 text-slate-500 hover:text-slate-700 dark:text-slate-300 text-sm transition-colors">
                    Skip to Next (no new/due cards)
                  </button>
                )}
                {showingAnswer && (
                  <button onClick={getHandler('onVocabReviewDone')}
                    className="w-full mt-4 py-3 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:bg-slate-600 text-slate-900 dark:text-white rounded-xl transition-colors">
                    Continue Story
                  </button>
                )}
              </>
            ...
            {actPhase === 'outcome' && currentLine && (
              <>
                ...
                {outcomePassed !== undefined && (
                  <div className={`w-full max-w-md mx-auto mb-4 p-4 rounded-xl text-center ${
                    outcomePassed
                      ? 'bg-green-100 dark:bg-green-900/30 border border-green-300 dark:border-green-700'
                      : 'bg-red-100 dark:bg-red-900/30 border border-red-300 dark:border-red-700'
                  }`}>
                    <p className={`text-lg font-bold ${
                      outcomePassed ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'
                    }`}>
                      {outcomePassed ? '✓ Success!' : '✗ Failed'}
                    </p>
                    {outcomeSubplot && (
                      <p className={`text-sm mt-1 ${
                        outcomePassed ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                      }`}>
                        {(subplots[outcomeSubplot]?.name || outcomeSubplot).replace('subplot_', '')}:
                        <span className="font-bold ml-1">
                          {outcomeDelta && outcomeDelta > 0 ? '+' : ''}{outcomeDelta || 0}
                        </span>
                      </p>
                    )}
                  </div>
        ...
        <div className="fixed inset-0 z-[60] bg-black/60 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 border border-red-600 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={getHandler('onCancelReset')} className="px-4 py-2 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:bg-slate-600 text-slate-900 dark:text-white rounded-lg transition-colors">Cancel</button>
              <button onClick={getHandler('onConfirmReset')} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-slate-900 dark:text-white rounded-lg transition-colors font-semibold">Reset Everything</button>
            </div>
          </div>
        </div>
      )}
      ...
      <SettingsDrawer isOpen={isSettingsOpen} dateshift={dateshift} cmsBaseUrl={cmsBaseUrl}
        ...
        onTapResetState={getHandler('onTapResetState')} onChangeCmsBase={getHandler('onChangeCmsBase')}
        mostOverdueCardInfo={props.mostOverdueCardInfo || null}
